...
fn main() -> std::io::Result<()> {
    ...
    println!("q5 signal cache server listening on {addr}");
    println!("send SIGHUP to pid {} to clear the cache", process::id());
        ...
        for fd in ready_clients {
            ...
            if finished {
                if let Some(mut client) = clients.remove(&fd) {
                    let response = file_response(&client.request, &mut cache);
