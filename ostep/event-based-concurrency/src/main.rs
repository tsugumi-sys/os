use nix::fcntl::{open, OFlag};
use nix::sys::select::{select, FdSet};
use nix::sys::signal::{self, SigHandler, Signal};
use nix::sys::stat::Mode;
use nix::unistd::{close, read};
use std::collections::HashMap;
use std::env;
use std::io::{Read, Write};
use std::net::{TcpListener, TcpStream};
use std::os::fd::{AsRawFd, BorrowedFd, IntoRawFd, RawFd};
use std::process;
use std::sync::atomic::{AtomicBool, Ordering};
use std::time::{SystemTime, UNIX_EPOCH};

static CLEAR_CACHE: AtomicBool = AtomicBool::new(false);

extern "C" fn handle_sighup(_: i32) {
    CLEAR_CACHE.store(true, Ordering::SeqCst);
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum ModeKind {
    TimeBlocking,
    TimeSelect,
    FileSelect,
}

struct Config {
    addr: String,
    mode: ModeKind,
}

fn main() {
    let config = parse_args();
    install_sighup_handler();

    let result = match config.mode {
        ModeKind::TimeBlocking => run_blocking_time_server(&config.addr),
        ModeKind::TimeSelect => run_select_server(&config.addr, RequestKind::Time),
        ModeKind::FileSelect => run_select_server(&config.addr, RequestKind::File),
    };

    if let Err(err) = result {
        eprintln!("server error: {err}");
        process::exit(1);
    }
}

fn parse_args() -> Config {
    let mut addr = String::from("127.0.0.1:7878");
    let mut mode = ModeKind::FileSelect;
    let mut args = env::args().skip(1);

    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--addr" => {
                addr = args
                    .next()
                    .unwrap_or_else(|| usage("--addr requires a value"));
            }
            "--mode" => {
                let value = args
                    .next()
                    .unwrap_or_else(|| usage("--mode requires a value"));
                mode = match value.as_str() {
                    "time-blocking" => ModeKind::TimeBlocking,
                    "time-select" => ModeKind::TimeSelect,
                    "file-select" => ModeKind::FileSelect,
                    _ => usage("unknown mode"),
                };
            }
            "-h" | "--help" => usage(""),
            _ => usage("unknown argument"),
        }
    }

    Config { addr, mode }
}

fn usage(message: &str) -> ! {
    if !message.is_empty() {
        eprintln!("{message}");
    }

    eprintln!(
        "usage: cargo run -- --mode <time-blocking|time-select|file-select> --addr 127.0.0.1:7878"
    );
    process::exit(if message.is_empty() { 0 } else { 2 });
}

fn install_sighup_handler() {
    let handler = SigHandler::Handler(handle_sighup);
    unsafe {
        signal::signal(Signal::SIGHUP, handler).expect("failed to install SIGHUP handler");
    }
}

fn run_blocking_time_server(addr: &str) -> std::io::Result<()> {
    let listener = TcpListener::bind(addr)?;
    println!("blocking time server listening on {addr}");

    for stream in listener.incoming() {
        let mut stream = stream?;
        stream.write_all(current_time_response().as_bytes())?;
    }

    Ok(())
}

#[derive(Debug, Clone, Copy)]
enum RequestKind {
    Time,
    File,
}

struct Client {
    stream: TcpStream,
    request: Vec<u8>,
}

fn run_select_server(addr: &str, kind: RequestKind) -> std::io::Result<()> {
    let listener = TcpListener::bind(addr)?;
    listener.set_nonblocking(true)?;
    println!("{kind:?} select server listening on {addr}");
    println!(
        "send SIGHUP to pid {} to clear the file cache",
        process::id()
    );

    let mut clients: HashMap<RawFd, Client> = HashMap::new();
    let mut cache: HashMap<String, Vec<u8>> = HashMap::new();

    loop {
        if CLEAR_CACHE.swap(false, Ordering::SeqCst) {
            cache.clear();
            println!("file cache cleared");
        }

        let listener_fd = listener.as_raw_fd();
        let mut readfds = FdSet::new();
        insert_fd(&mut readfds, listener_fd);

        let mut max_fd = listener_fd;
        for fd in clients.keys().copied() {
            insert_fd(&mut readfds, fd);
            max_fd = max_fd.max(fd);
        }

        match select(max_fd + 1, Some(&mut readfds), None, None, None) {
            Ok(_) => {}
            Err(nix::errno::Errno::EINTR) => continue,
            Err(err) => return Err(std::io::Error::from_raw_os_error(err as i32)),
        }

        if contains_fd(&readfds, listener_fd) {
            loop {
                match listener.accept() {
                    Ok((stream, _)) => {
                        stream.set_nonblocking(true)?;
                        clients.insert(
                            stream.as_raw_fd(),
                            Client {
                                stream,
                                request: Vec::new(),
                            },
                        );
                    }
                    Err(err) if err.kind() == std::io::ErrorKind::WouldBlock => break,
                    Err(err) => return Err(err),
                }
            }
        }

        let ready_clients: Vec<RawFd> = clients
            .keys()
            .copied()
            .filter(|fd| contains_fd(&readfds, *fd))
            .collect();

        for fd in ready_clients {
            let finished = if let Some(client) = clients.get_mut(&fd) {
                read_client_request(client)?
            } else {
                false
            };

            if finished {
                if let Some(mut client) = clients.remove(&fd) {
                    let response = match kind {
                        RequestKind::Time => current_time_response().into_bytes(),
                        RequestKind::File => file_response(&client.request, &mut cache),
                    };
                    client.stream.write_all(&response)?;
                }
            }
        }
    }
}

fn read_client_request(client: &mut Client) -> std::io::Result<bool> {
    let mut buf = [0_u8; 1024];

    loop {
        match client.stream.read(&mut buf) {
            Ok(0) => return Ok(true),
            Ok(n) => {
                client.request.extend_from_slice(&buf[..n]);
                if client.request.contains(&b'\n') {
                    return Ok(true);
                }
            }
            Err(err) if err.kind() == std::io::ErrorKind::WouldBlock => return Ok(false),
            Err(err) => return Err(err),
        }
    }
}

fn current_time_response() -> String {
    let now = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system clock is before unix epoch");
    format!("unix_time_seconds {}\n", now.as_secs())
}

fn file_response(request: &[u8], cache: &mut HashMap<String, Vec<u8>>) -> Vec<u8> {
    let path = match parse_path(request) {
        Ok(path) => path,
        Err(message) => return format!("ERR {message}\n").into_bytes(),
    };

    let contents = if let Some(contents) = cache.get(&path) {
        contents.clone()
    } else {
        match read_file_with_syscalls(&path) {
            Ok(contents) => {
                cache.insert(path.clone(), contents.clone());
                contents
            }
            Err(err) => return format!("ERR {err}\n").into_bytes(),
        }
    };

    let mut response = format!("OK {} bytes\n", contents.len()).into_bytes();
    response.extend_from_slice(&contents);
    if !contents.ends_with(b"\n") {
        response.push(b'\n');
    }
    response
}

fn parse_path(request: &[u8]) -> Result<String, &'static str> {
    let line = request
        .split(|byte| *byte == b'\n')
        .next()
        .ok_or("empty request")?;
    let path = std::str::from_utf8(line)
        .map_err(|_| "request path must be utf-8")?
        .trim();

    if path.is_empty() {
        return Err("request path is empty");
    }

    if path.starts_with('/') || path.contains("..") {
        return Err("only relative paths below the current directory are allowed");
    }

    Ok(path.to_string())
}

fn read_file_with_syscalls(path: &str) -> std::io::Result<Vec<u8>> {
    let fd = open(path, OFlag::O_RDONLY, Mode::empty()).map_err(nix_to_io)?;
    let mut contents = Vec::new();
    let mut buf = [0_u8; 4096];

    loop {
        match read(&fd, &mut buf) {
            Ok(0) => break,
            Ok(n) => contents.extend_from_slice(&buf[..n]),
            Err(err) => {
                let raw_fd = fd.into_raw_fd();
                let _ = close(raw_fd);
                return Err(nix_to_io(err));
            }
        }
    }

    let raw_fd = fd.into_raw_fd();
    close(raw_fd).map_err(nix_to_io)?;
    Ok(contents)
}

fn insert_fd(set: &mut FdSet<'_>, fd: RawFd) {
    unsafe {
        set.insert(BorrowedFd::borrow_raw(fd));
    }
}

fn contains_fd(set: &FdSet<'_>, fd: RawFd) -> bool {
    unsafe { set.contains(BorrowedFd::borrow_raw(fd)) }
}

fn nix_to_io(err: nix::errno::Errno) -> std::io::Error {
    std::io::Error::from_raw_os_error(err as i32)
}
